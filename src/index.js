...
//It will make the buttons workable [name,NFTWIDTH,HEIGHTAND DEPTH]
...
function mintNFT() {
    ...
    if (typeof window.ethereum == "undefined") {
      ...
      web3.eth.requestAccounts().then((accounts)=>{
        contract.methods.mint(nft_name,nft_width,nft_height,nft_depth,nft_x,nft_y,nft_z).send({from:accounts[0],value:"10",}).then((data)=>{
...
polygon.then((result) => {
    result.nft.forEach((object, index) => {
      if (index<=result.supply) {
        const geometry_cone = new THREE.BoxGeometry(object.w,object.h,object.d);
        const material_cone = new THREE.MeshPhongMaterial({ color: 0x1be3ef });
